...
namespace Zuby.ADGV
{
	...
	public class ColumnHeaderCell : DataGridViewColumnHeaderCell
	{
		...
		public ColumnHeaderCell(DataGridViewColumnHeaderCell oldCell, bool filterEnabled)
		{
			...
			{
				_filterButtonImageSize    = oldCell.OwningColumn.DataGridView.LogicalToDeviceUnits(_filterButtonImageSize);
		...
		{
			MenuStrip.DisableCustomMenueItems();
		}
		...
		/// </summary>
		public Zuby.SortType ActiveSortType
		{
			get
			{
				if (MenuStrip != null && FilterAndSortEnabled)
		...
		private void RefreshImage()
		{
			...
			else
			{
				...
				{
					if (ActiveSortType == Zuby.SortType.None)
						_filterImage = Properties.Resources.ColumnHeader_UnFiltered;
					else if (ActiveSortType == Zuby.SortType.ASC)
