...
namespace Zuby.ADGV
{
	public class AdvancedDataGridViewEventArgs : EventArgs
	{
		public string Filter { get; set; }

		public AdvancedDataGridViewEventArgs(String filter) : base()
		{
			Filter = filter;
		}
	}
	...
	public class AdvancedDataGridView : DataGridView
	{
		...
		public List<ColumnFilterSelection> FilterSelektions = new List<ColumnFilterSelection>();
		public List<ColumnSortSelection> SortSelektion = new List<ColumnSortSelection>();
		...
		/// <summary>
		/// siehe: https://stackoverflow.com/questions/34344499/invalidoperationexception-this-operation-cannot-be-performed-while-an-auto-fill
		/// "Dieser Vorgang kann nicht ausgeführt werden, während die Größe einer automatisch gefüllten Spalte geändert wird. WinForms"
		/// </summary>
		/// <param name="e"></param>
		...
		/// <summary>
		/// Diasables the custom menue items.
		/// </summary>
		public void DiasableCustomMenueItems()
		{
			//foreach (ColumnHeaderCell c in FilterableCells)
			//    c.DisableCustomMenueItems();
		}
			...
			private set
			{
				...
				if (old != _sortString)
				{
					...
					else
					{
						...
						{
							((BindingSource)this.DataSource).Sort = _sortString;
						}
					}
				}
			}
			...
			private set
			{
				...
				if (old != _filterString)
				{
					...
					if (FilterStringChanged != null)
						FilterStringChanged(this, new AdvancedDataGridViewEventArgs(_filterString));
					else
					{
						if (this.DataSource is BindingSource)
						{
							((BindingSource)this.DataSource).Filter = _filterString;
		...
		{
			this.FilterSelektions.Clear();
			this._customFilterList.Clear();
			foreach (ColumnHeaderCell c in FilterableCells)
			{
				c.CleanFilter();
			}
		...
		private List<CustomFilter> _customFilterList = new List<CustomFilter>();
		public List<CustomFilter> CustomFilterList
		{
			get { return _customFilterList; }
		}
		/// <summary>
		...
		private string BuildFilterString()
		{
			...
			string appx = "";
			FilterSelektions.Clear();
			_customFilterList.Clear();
			...
			foreach (string filterOrder in _filterOrderList)
			{
				...
				if (Column != null)
				{
					...
					if (cell != null)
					{
						String dataPropertyName = cell.MenuStrip.DataPropertyName;
						if (cell.FilterAndSortEnabled && cell.ActiveFilterType != MenuStrip.FilterType.None && !String.IsNullOrEmpty(cell.FilterString))
						{
							sb.AppendFormat(appx + "(" + cell.FilterString + ")", dataPropertyName);
							appx = " AND ";
						}
						if (cell.MenuStrip.ColumnFilterSelection != null &&
							cell.MenuStrip.ColumnFilterSelection.SelectedValues != null &&
							cell.MenuStrip.ColumnFilterSelection.SelectedValues.Count > 0)
						{
							FilterSelektions.Add(cell.MenuStrip.ColumnFilterSelection);
						}
						if (cell.MenuStrip.ActualCustomFilter != null)
						{
							_customFilterList.Add(cell.MenuStrip.ActualCustomFilter);
						}
		...
		/// <param name="e"></param>
		public void cell_FilterPopup(object sender, ColumnHeaderCellEventArgs e)
		{
			if (Columns.Contains(e.Column))
			{
				MenuStrip filterMenu = e.FilterMenu;
				DataGridViewColumn column = e.Column;
				if (filterMenu.DataType == null)
				{
					filterMenu.DataType = column.ValueType;
				}
				filterMenu.MaxEntries = this.MaxEntries;
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		public void cell_FilterChanged(object sender, ColumnHeaderCellEventArgs e)
		{
			if (Columns.Contains(e.Column))
			{
				MenuStrip filterMenu = e.FilterMenu;
		...
		private string BuildSortString()
		{
			...
			foreach (string sortOrder in _sortOrderList)
			{
				...
				if (column != null)
				{
					...
					if (cell != null)
					{
						if (cell.FilterAndSortEnabled && cell.ActiveSortType != Zuby.SortType.None)
						{
							sb.AppendFormat(appx + cell.SortString, column.DataPropertyName);
							appx = ", ";
							SortSelektion.Add(new ColumnSortSelection(column.DataPropertyName, cell.ActiveSortType, cell.MenuStrip.DataType));
						}
					}
				}
			}
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		public void cell_SortChanged(object sender, ColumnHeaderCellEventArgs e)
		{
			if (Columns.Contains(e.Column))
			{
				MenuStrip filterMenu = e.FilterMenu;
				...
				_sortOrderList.Remove(column.Name);
				if (filterMenu.ActiveSortType != Zuby.SortType.None)
