...
namespace Zuby.ADGV
{
	...
	public partial class MenuStrip : ContextMenuStrip
	{
		...
		private const string ConvertYearMonthDayFormat = "d";
		private const string ConvertYearMonthDayHourMinuteSecondFormat = "G";
		public ColumnHeaderCell _headerCell;
		private CustomFilter _actualFilter = null;
		private Dictionary<String, CustomFilter> _savedCustomFilter = new Dictionary<String, CustomFilter>();

		public Zuby.CustomFilter ActualCustomFilter
		{
			get { return _actualFilter; }
		}

        [DefaultValue(null)]
        public Zuby.ADGV.ColumnHeaderCell HeaderCell
		{
			get { return _headerCell; }
			set { _headerCell = value; }
		}
			...
			: base()
		{
			...
			this.customSearchTextBox.TextChanged += new EventHandler(customSearchTextBox_TextChanged);
			this.customSearchTextBox.KeyUp += new KeyEventHandler(customSearchTextBox_KeyUp);
		}

		/// <summary>
		/// Handles the KeyUp event of the customSearchTextBox control.
		/// </summary>
		/// <param name="sender">The source of the event.</param>
		/// <param name="e">The <see cref="System.Windows.Forms.KeyEventArgs"/> instance containing the event data.</param>
		void customSearchTextBox_KeyUp(object sender, KeyEventArgs e)
		{
			if (e.Control)
			{
				if (e.KeyCode == Keys.Insert || e.KeyCode == Keys.V)
				{
					customSearchTextBox.Paste();
				}
			}
		...
		private TreeNodeItemSelector[] _savedChecklist = null;
		/// <summary>
		/// Handles the TextChanged event of the customSearchTextBox control.
		/// </summary>
		/// <param name="sender">The source of the event.</param>
		/// <param name="e">The <see cref="System.EventArgs"/> instance containing the event data.</param>
		void customSearchTextBox_TextChanged(object sender, EventArgs e)
		{
			if (_savedChecklist != null)
			{
				RestoreSavedNodes();
				if (!String.IsNullOrEmpty(customSearchTextBox.Text))
				{
					String searchText = customSearchTextBox.Text.ToUpper();
					checkList.BeginUpdate();
					checkList.Nodes.Clear();
					checkList.Nodes.AddRange(_savedChecklist.Where(p =>
						p.Value != null &&
						!String.IsNullOrEmpty(p.Value.ToString()) &&
						p.Value.ToString().ToUpper().StartsWith(searchText)
						).ToArray());
		...
		/// necessary for the image left of ToolStripTextbox
		/// </summary>
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			for (int i = 0; i < this.Items.Count; i++)
			{
				try
				{
					...
					{
						float x = 5;
						float y = this.Items[i].Bounds.Y + ((this.Items[i].Bounds.Height / 2) - 8);
						e.Graphics.DrawImage(this.Items[i].Image, x, y);
					}
				}
				catch { }
			}
		...
		private static ImageList _CheckListStateImages = null;
		/// <summary>
		...
		private ImageList GetCheckListStateImages()
		{
			if (_CheckListStateImages == null)
			{
				...
				Bitmap unCheckImg = null; // new Bitmap(16, 16);
				Bitmap checkImg = null; // new Bitmap(16, 16);
				Bitmap mixedImg = null; //new Bitmap(16, 16);
				...
				using (Bitmap img = new Bitmap(16, 16))
				{
					using (Graphics g = Graphics.FromImage(img))
					{
						...
						try
						{
							CheckBoxRenderer.DrawCheckBox(g, p, System.Windows.Forms.VisualStyles.CheckBoxState.UncheckedNormal);
							unCheckImg = (Bitmap)img.Clone();
						}
						catch
						{
							unCheckImg = new Bitmap(16, 16);
						}
						try
						{
							CheckBoxRenderer.DrawCheckBox(g, p, System.Windows.Forms.VisualStyles.CheckBoxState.CheckedNormal);
							checkImg = (Bitmap)img.Clone();
						}
						catch
						{
							checkImg = new Bitmap(16, 16);
						}
						try
						{
							CheckBoxRenderer.DrawCheckBox(g, p, System.Windows.Forms.VisualStyles.CheckBoxState.MixedNormal);
							mixedImg = (Bitmap)img.Clone();
						}
						catch
						{
							mixedImg = new Bitmap(16, 16);
						}
					}
				}
		...
		/// </value>
		[DefaultValue(null)]
		public ColumnFilterSelection ColumnFilterSelection { get; set; }
		...
		/// <summary>
		/// Disables the custom menue items.
		/// </summary>
		public void DisableCustomMenueItems()
		{
			customFilterMenuItem.Visible = false;
		}
		...
		{
			this.customSearchTextBox.TextChanged -= new EventHandler(customSearchTextBox_TextChanged);
			this.customSearchTextBox.Text = String.Empty;
			this.customSearchTextBox.TextChanged += new EventHandler(customSearchTextBox_TextChanged);
			for (int i = 2; i < customFilterLastFiltersListMenuItem.DropDownItems.Count - 1; i++)
			{
				(customFilterLastFiltersListMenuItem.DropDownItems[i] as ToolStripMenuItem).Checked = false;
			...
			_activeFilterType = FilterType.None;
			SetNodesCheckState(checkList.Nodes, true);
			if (_actualFilter != null && !_savedCustomFilter.ContainsKey(FilterString))
			{
				_savedCustomFilter.Add(FilterString, _actualFilter);
			}
		...
		private void SetCheckListFilter()
		{
			...
			else
			{
				...
				if (checkList.Nodes.Count > 1 || selectAllNode == null)
				{
					if (selectAllNode != null && selectAllNode.Checked)
					...
					if (checkList.Nodes.Count > 2 || selectAllNode == null)
					{
						...
						if (filter.Length > 0)
						{
							...
							{
								FilterString += " OR ";
								this.ColumnFilterSelection.SelectedValues.Insert(0, null);
							}
						...
						else if (selectAllNode != null && selectAllNode.Checked)
						{
							this.ColumnFilterSelection.SelectedValues.Add(selectAllNode.Value);
						}
					}
				}
		...
		public String DataPropertyName
		{
			get
			{
				DataGridViewColumn Column = HeaderCell.OwningColumn;
				String dataPropertyName = Column.DataPropertyName;
				if (Column is DataGridViewAutoFilter.DataGridViewAutoFilterTextBoxColumn)
				{
					dataPropertyName = ((DataGridViewAutoFilter.DataGridViewAutoFilterTextBoxColumn)Column).DataPropertyName;
				}
		...
		/// <returns></returns>
		private string BuildNodesFilterString(IEnumerable<TreeNodeItemSelector> nodes, List<Object> selectedValueList = null)
		{
			StringBuilder sb = new StringBuilder("");
			string appx = (DataType == typeof(DateTime) || DataType == typeof(DateTime?)) ? " OR " : ", ";
			DataGridViewColumn Column = HeaderCell.OwningColumn;
			String dataPropertyName = this.DataPropertyName;
			this.ColumnFilterSelection = new ColumnFilterSelection();
			_actualFilter = null;

			if (selectedValueList == null)
			{
				selectedValueList = new List<Object>();
			}
			...
			if (nodes != null && nodes.Count() > 0)
			{
					...
					foreach (TreeNodeItemSelector n in nodes)
					{
						if (n.Checked && (n.Nodes.AsParallel().Cast<TreeNodeItemSelector>().Where(sn => sn.CheckState != CheckState.Unchecked).Count() == 0))
						{
							...
							if (IsFilterDateAndTimeEnabled)
							{
								...
								selectedValueList.Add(dt);
							}
							else
							{
								sb.Append("(Convert([{0}], 'System.String') LIKE '%" + String.Format("{0:" + ConvertYearMonthDayFormat + "}", dt) + "%')" + appx);
								selectedValueList.Add(dt.Date);
							}
						}
						else if (n.CheckState != CheckState.Unchecked && n.Nodes.Count > 0)
						{
							string subnode = BuildNodesFilterString(n.Nodes.AsParallel().Cast<TreeNodeItemSelector>().Where(sn => sn.CheckState != CheckState.Unchecked), selectedValueList);
							if (subnode.Length > 0)
				...
				else if (DataType == typeof(bool))
				{
					foreach (TreeNodeItemSelector n in nodes)
					{
						...
						selectedValueList.Add(n.Value.ToString());
						break;
					}
				}
				else if (DataType == typeof(Int32) || DataType == typeof(Int64) || DataType == typeof(Int16) ||
			...
			this.ColumnFilterSelection.DataType = DataType;
			this.ColumnFilterSelection.DataPropertyName = dataPropertyName;
			this.ColumnFilterSelection.SelectedValues = selectedValueList;
		...
		private void BuildNodes(IEnumerable<DataGridViewCell> vals)
		{
			...
			if (vals != null)
			{
				...
				allnode.NodeFont = new Font(checkList.Font, FontStyle.Bold);
				checkList.Nodes.Add(allnode);
				if (valCount > _maxEntries)
				{
					MessageBox.Show("There are more than " +  _maxEntries.ToString() +" rows, filtering would take to much time...", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
				}
				else if (valCount > 0)
				{
					...
					else if (DataType == typeof(DateTime?))
					{
						var years =
							from year in nonulls
							group year by ((DateTime?)year.Value).Value.Year into cy
							orderby cy.Key ascending
							select cy;
						...
						foreach (var year in years)
						{
							...
							var months =
								from month in year
								group month by ((DateTime?)month.Value).Value.Month into cm
								orderby cm.Key ascending
								select cm;
							...
							foreach (var month in months)
							{
								...
								var days =
									from day in month
									group day by ((DateTime?)day.Value).Value.Day into cd
									orderby cd.Key ascending
									select cd;
								...
								foreach (var day in days)
								{
									...
									else
									{
										...
										var hours =
											from hour in day
											group hour by ((DateTime?)hour.Value).Value.Hour into ch
											orderby ch.Key ascending
											select ch;
										...
										foreach (var hour in hours)
										{
											...
											var mins =
												from min in hour
												group min by ((DateTime?)min.Value).Value.Minute into cmin
												orderby cmin.Key ascending
												select cmin;
											...
											foreach (var min in mins)
											{
												...
												var secs =
													from sec in min
													group sec by ((DateTime?)sec.Value).Value.Second into cs
													orderby cs.Key ascending
													select cs;
					...
					{
						foreach (var v in nonulls.GroupBy(c => c.FormattedValue).OrderBy(g => g.Key))
						{
					...
					{
						foreach (var v in nonulls.Where(c => !String.IsNullOrWhiteSpace(c.Value as string)).GroupBy(c => c.Value).OrderBy(g => g.Key))
						{
							TreeNodeItemSelector node = TreeNodeItemSelector.CreateNode(v.First().FormattedValue.ToString(), v.Key, CheckState.Checked, TreeNodeItemSelector.CustomNodeType.Default);
							checkList.Nodes.Add(node);
						}
		...
		private void NodeCheckChange(TreeNodeItemSelector node)
		{
			...
			else
			{
				...
				TreeNodeItemSelector tis = GetSelectAllNode();
				if (tis != null)
				{
					tis.CheckState = state;
				}
		...
		{
			_savedChecklist = new TreeNodeItemSelector[checkList.Nodes.Count];
			int i = 0;
			foreach (TreeNodeItemSelector n in checkList.Nodes)
			{
				_savedChecklist[i] = n.Clone();
				_savedChecklist[i].Checked = false;
				if (_savedChecklist[i].Nodes != null && _savedChecklist[i].Nodes.Count > 0)
					SetNodesCheckState(_savedChecklist[i].Nodes, false);
				i++;
			}
		}
		...
		/// <summary>
		/// Restores the saved nodes.
		/// </summary>
		private void RestoreSavedNodes()
		{
			checkList.Nodes.Clear();
			if (_savedChecklist != null)
				checkList.Nodes.AddRange(_savedChecklist);
		}
		...
		private void SetCustomFilter(int filtersMenuItemIndex)
		{
			...
			if (_savedCustomFilter.ContainsKey(filterstring))
			{
				_actualFilter = _savedCustomFilter[filterstring];
			}
		...
		private void customFilterMenuItem_Click(object sender, EventArgs e)
		{
			...
			flt.DataType = this.DataType;

			if (flt.ShowDialog(this) == DialogResult.OK)
			{
				//add the new Filter presets
				_actualFilter                  = flt.ActualFilter;
				if (_actualFilter != null && _actualFilter.FExpression != FilterExpression.nothing)
				{
					_actualFilter.DataType = this.DataType;
					_actualFilter.DataPropertyName = this.DataPropertyName;
		...
		private void ResizeBox(int w, int h)
		{
			...
			button_ok.Location = new Point(Scale(w2 - 35 - 164, scalingfactor), 0);
			button_cancel.Location = new Point(Scale(w2 - 35 - 79, scalingfactor), 0);
		...
		//	int minusHeight = screenHeight > 1080 ? 185 : 65;
		//	sortASCMenuItem.Width = w - 1;
		//	sortDESCMenuItem.Width = w - 1;
		//	cancelSortMenuItem.Width = w - 1;
		//	cancelFilterMenuItem.Width = w - 1;
		//	customFilterMenuItem.Width = w - 1;
		//	customFilterLastFiltersListMenuItem.Width = w - 1;
		//	checkFilterListControlHost.Size = new System.Drawing.Size(w - 35, h - minusHeight);
		//	checkFilterListPanel.Size = new System.Drawing.Size(w - 35, h - minusHeight);
		//	checkList.Bounds = new Rectangle(4, 4, w - 35 - 8, h - minusHeight - 8);
