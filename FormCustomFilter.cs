...
namespace Zuby.ADGV
{
	internal partial class FormCustomFilter : Form
	{
		...
		private string ConvertYearMonthDayFormat = "d"; // because DateTime.ToString() is formatted using the general date and time format specifier ('G'). 
		private string ConvertYearMonthDayHourMinuteFormat = "G"; // because DateTime.ToString() is formatted using the general date and time format specifier ('G').
			...
			: this()
		{
			...
			_textStrings.Add("EQUALS", new CustomFilter("equals", FilterExpression.equals));
			_textStrings.Add("DOES_NOT_EQUAL", new CustomFilter("does not equal", FilterExpression.does_not_equal));
			_textStrings.Add("EARLIER_THAN", new CustomFilter("earlier than", FilterExpression.earlier_than));
			_textStrings.Add("LATER_THAN", new CustomFilter("later than", FilterExpression.later_than));
			_textStrings.Add("BETWEEN", new CustomFilter("between", FilterExpression.between));
			_textStrings.Add("GREATER_THAN", new CustomFilter("greater than", FilterExpression.greater_than));
			_textStrings.Add("GREATER_THAN_OR_EQUAL_TO", new CustomFilter("greater than or equal to", FilterExpression.greater_than_or_equal_to));
			_textStrings.Add("LESS_THAN", new CustomFilter("less than", FilterExpression.less_than));
			_textStrings.Add("LESS_THAN_OR_EQUAL_TO", new CustomFilter("less than or equal to", FilterExpression.less_than_or_equal_to));
			_textStrings.Add("BEGINS_WITH", new CustomFilter("begins with", FilterExpression.begins_with));
			_textStrings.Add("DOES_NOT_BEGIN_WITH", new CustomFilter("does not begin with", FilterExpression.does_not_begin_with));
			_textStrings.Add("ENDS_WITH", new CustomFilter("ends with", FilterExpression.ends_with));
			_textStrings.Add("DOES_NOT_END_WITH", new CustomFilter("does not end with", FilterExpression.does_not_end_with));
			_textStrings.Add("CONTAINS", new CustomFilter("contains", FilterExpression.contains));
			_textStrings.Add("DOES_NOT_CONTAIN", new CustomFilter("does not contain", FilterExpression.does_not_contain));
			...
			else if (dataType == typeof(Int32) || dataType == typeof(Int64) || dataType == typeof(Int16) || dataType == typeof(Int32?) || dataType == typeof(Int64?) || dataType == typeof(Int16?) ||
			...
			else if (dataType == typeof(Single) || dataType == typeof(Double) || dataType == typeof(Decimal) || dataType == typeof(Decimal?))
		...
		/// </summary>
		public Zuby.CustomFilter ActualFilter
		{
			get { return _actualFilter; }
			private set { _actualFilter = value; }
		}
		...
		private string BuildCustomFilter(FilterType filterType, bool filterDateAndTimeEnabled, string filterTypeConditionText, Control control1, Control control2)
		{
			...
			filterString = column;
			object o = _textStrings[filterTypeConditionText.Replace(" ","_").ToUpper()];
			if (o is CustomFilter)
			{
				_actualFilter = (CustomFilter)o;
			}
			else
				_actualFilter = new CustomFilter("Unknown operation", FilterExpression.nothing);
			switch (filterType)
			{
				case FilterType.DateTime:
					DateTime dt = ((DateTimePicker)control1).Value;
					_actualFilter.Value1 = dt;
					...
					{
						if (filterDateAndTimeEnabled)
							filterString = "Convert([{0}], 'System.String') NOT LIKE '%" + String.Format("{0:" + ConvertYearMonthDayFormat + "}", dt) + "%'";
						else
							filterString = "Convert([{0}], 'System.String') NOT LIKE '%" + String.Format("{0:" + ConvertYearMonthDayFormat + "}", dt) + "%'";
					}
					break;
				...
				case FilterType.Float:
					...
					_actualFilter.Value1 = getCorrectDataType(num);
					...
					{
						_actualFilter.Value2 = getCorrectDataType(control2.Text);
				...
				default:
					...
					_actualFilter.Value1 = getCorrectDataType(control1.Text);
					if (filterTypeConditionText == _textStrings["EQUALS"].ToString())
		...
		/// <summary>
		/// Gets the type of the correct data.
		/// </summary>
		/// <param name="s">The s.</param>
		/// <returns></returns>
		private object getCorrectDataType(String s)
		{
			if (this.DataType == typeof(System.Int16) || this.DataType == typeof(System.Int16?))
			{
				System.Int16 v = 0;
				System.Int16.TryParse(s, out v);
				return v;
			}
			else if (this.DataType == typeof(System.Int32) || this.DataType == typeof(System.Int32?))
			{
				System.Int32 v = 0;
				System.Int32.TryParse(s, out v);
				return v;
			}
			else if (this.DataType == typeof(System.Int64) || this.DataType == typeof(System.Int64?))
			{
				System.Int64 v = 0;
				System.Int64.TryParse(s, out v);
				return v;
			}
			else if (this.DataType == typeof(float) || this.DataType == typeof(double))
			{
